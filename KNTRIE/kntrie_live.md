...
### 9.1 Allocation table consolidation — DONE
- ALLOC_CLASS_TABLE single canonical source, sizeof-derived NUM_ALLOC_CLASSES
- round_up_u64 linear scan of table, removed BIN_SIZES/NUM_BINS/FREE_MAX/MIDPOINT_MIN_BIAS

### 9.2 compact vals accessors — val offset from alloc_u64 — DONE
- vals_mut/vals/bool_vals_mut/bool_vals take only node (read offset from header)
- val_ptr takes (node, pos) only
- New helpers: val_block_u64, capacity_for, set_val_offset, total_from_val_offset
- has_room takes (entries, header*) instead of (entries, alloc_u64)

### 9.3 compact make_leaf/for_each/destroy — DONE
- make_leaf uses set_val_offset
- for_each/destroy_and_dealloc/dealloc_node_only use new accessors + total_from_val_offset

### 9.4 compact insert — DONE
- ASSIGN path: vals_mut(node) instead of vals_mut(node, entries, hs)
- Grow-realloc: set_val_offset, total_from_val_offset for dealloc
- In-place: two memmoves always, no triple memmove, no tail>0 guards

### 9.5 compact erase — DONE
- Shrink-realloc: total_from_val_offset, set_val_offset
- In-place: two memmoves always, no triple memmove, no tail>0 guards

### 9.6 kntrie_ops value_ref_at/bool_ref_at/leaf_first_after — DONE
- vals_mut(node) instead of vals_mut(node, entries, hs)
- val_ptr(node, pos) instead of val_ptr(node, entries, hs, pos)
- bool_vals_mut(node) instead of bool_vals_mut(node, entries, hs)

### 9.7 kntrie_ops compact leaf dealloc sites — DONE
- convert_to_bitmask_tagged dealloc uses CO::total_from_val_offset
- FREE_MAX renamed to MAX_NODE_ALLOC_U64
- All other dealloc sites (split_skip_at, dealloc_subtree, dealloc_coalesced_node) are bitmask nodes — alloc_u64() correct as-is
- split_skip_at line 617: hdr->alloc_u64() → total_from_val_offset (need CO type)
- convert_to_bitmask_tagged line 674: same
- dealloc_subtree compact leaf path: same (dispatched via depth_switch, CO available)
