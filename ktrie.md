...
[Figure: A B-tree node containing a sorted array of keys with child pointers between them. The sorted array is contiguous in memory — a single cache-line fetch can cover multiple comparisons. This is the density property that compact nodes inherit.]
...
[Figure: bitmask node layout showing bitmap, sentinel, child slots, EOS, and skip bytes. Same key sets from §1, shown after the compact node has overflowed and split — the large-N case. Contrast with §3.3: same data, different structural representation depending on density.]
...
[Figure: KNTRIE compact node layout (sorted keys + values); KSTRIE compact node layout (L[]/F[]/O[]/keysuffix/values with reconstruction walkthrough). Same key sets from §1 shown as they would be stored when the entire dataset fits in a single compact node — the small-N case.]
