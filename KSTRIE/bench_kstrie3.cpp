...
static void bench_one(const Workload& w, std::vector<Row>& rows, bool verbose) {
    ...
    {
        using TrieT = gteitelbaum::kstrie<std::string,
                        gteitelbaum::identity_char_map,
                        TrackingAlloc<uint64_t>>;
        g_alloc_total = 0;
        TrieT t;
        for (size_t i = 0; i < n; ++i) t.insert(w.keys[i], std::to_string(i));
        kstrie_mem = g_alloc_total;
    }
    {
        using UMapT = std::unordered_map<std::string, std::string,
                        std::hash<std::string>, std::equal_to<std::string>,
                        TrackingAlloc<std::pair<const std::string, std::string>>>;
        g_alloc_total = 0;
        UMapT m;
        m.reserve(n);
        for (size_t i = 0; i < n; ++i) m.emplace(w.keys[i], std::to_string(i));
    ...
    for (int t = 0; t < trials_for(n); ++t) {
        ...
        // ---- kstrie ----
        {
            gteitelbaum::kstrie<std::string> trie;
            double t0 = now_ms();
            for (auto i : insert_order) trie.insert(w.keys[i], std::to_string(i));
            k_ins = std::min(k_ins, now_ms() - t0);

            { uint64_t s = 0; double ti = now_ms();
              for (auto it = trie.begin(); it != trie.end(); ++it) s += it.value().size();
              k_iter = std::min(k_iter, now_ms() - ti); do_not_optimize(s); }

            uint64_t cs = 0;
            double t1 = now_ms();
            for (int r = 0; r < w.find_iters; ++r)
                for (auto i : fnd_idx[r]) { auto* v = trie.find(w.find_fnd[i]); cs += v ? v->size() : 0; }
            ...
            double t1n = now_ms();
            for (int r = 0; r < w.find_iters; ++r)
                for (auto i : nf_idx[r]) { auto* v = trie.find(w.find_nf[i]); cs += v ? v->size() : 0; }
            ...
            if (true) {
                ...
                for (int r = 0; r < w.find_iters; ++r) {
                    ...
                    for (auto i : erase_order) trie.insert(w.erase_keys[i], std::to_string(i));
                }
        ...
        // ---- unordered_map ----
        {
            ...
            m.reserve(n);
            double t0 = now_ms();
            for (auto i : insert_order) m.emplace(w.keys[i], std::to_string(i));
            u_ins = std::min(u_ins, now_ms() - t0);

            { uint64_t s = 0; double ti = now_ms();
              for (auto& [k,v] : m) s += v.size();
              u_iter = std::min(u_iter, now_ms() - ti); do_not_optimize(s); }

            uint64_t cs = 0;
            double t1 = now_ms();
            for (int r = 0; r < w.find_iters; ++r)
                for (auto i : fnd_idx[r]) { auto it = m.find(w.find_fnd[i]); cs += (it!=m.end()) ? it->second.size() : 0; }
            ...
            double t1n = now_ms();
            for (int r = 0; r < w.find_iters; ++r)
                for (auto i : nf_idx[r]) { auto it = m.find(w.find_nf[i]); cs += (it!=m.end()) ? it->second.size() : 0; }
            ...
            if (true) {
                ...
                for (int r = 0; r < w.find_iters; ++r) {
                    ...
                    for (auto i : erase_order) m.emplace(w.erase_keys[i], std::to_string(i));
                }
