...
10. 🔶 Remove snapshot-only APIs
   **Done (kstrie.hpp — all snapshot code removed):**
   - iter_upper_bound, iter_lower_bound + find_ge_impl
   - iter_next + find_next_impl, iter_prev + find_prev_impl
   - find_max_impl, iter_min / iter_max, find_value
   - find_min_impl, find_min_impl_tail
   - iter_prefix_bounds + prefix_result
   - find_prefix_first_in_compact, find_prefix_past_in_compact
   - prefix() rewritten as single-walk (no double descent)
   **Remaining (kstrie_impl.hpp, bottom-to-top):**
   - erase_when_node (~1791-1923)
   - erase_when public method (~965-1000)
   - modify_or_insert_dispatch (~894-964)
